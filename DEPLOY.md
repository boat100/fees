...
**重要说明**：本系统使用 SQLite 数据库（better-sqlite3），这是一个需要本地编译的原生 Node.js 模块。

### 在线预览的限制

Coze 在线预览环境可能存在以下限制：
1. **原生模块兼容性**：better-sqlite3 需要针对目标平台编译
2. **数据持久化**：在线预览环境的数据可能无法持久保存
3. **构建环境**：缺少编译原生模块所需的 Python 和 C++ 编译工具

### 推荐方案

**生产环境请使用 Docker 部署到自己的服务器**，这样可以：
- ✅ 完全控制运行环境
- ✅ 数据持久化保存
- ✅ 支持 ARM64 架构（N1 盒子）
- ✅ 可以自定义配置
...
如果在线预览遇到 "Internal Server Error" 或数据操作失败，说明原生模块未能正确加载。请：
1. 使用 Docker 部署到本地或服务器
2. 或联系平台支持了解原生模块支持情况
